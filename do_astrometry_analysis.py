...
def do_nebula_pm_analysis(filename_stars_with_nebulae, nebulae_data):
    ...
    unique_nebulae = np.unique(stars_with_nebulae) # TODO: unique, counts = np.unique(x, return_counts=True)
        ...
        if np.size(arg_with_neb) >= min_stars_to_analyse_nebula:
            ...
            if pm_length_ratio < max_ratio_of_pm_to_be_ok:
                ...
                for star_index in list(arg_with_neb[0]):
                    ...
                    angle_pm_star_neb = np.arccos((pmra_star * pmra_neb_avg + pmdec_star * pmdec_neb_avg) / pm_neb_length / pm_star_length) / np.pi * 180
                    ratio_pm_star_neb = max(pm_star_length, pm_neb_length) / min(pm_star_length, pm_neb_length)

                    if abs(angle_pm_star_neb) < max_angle_between_star_and_neb_pm_to_be_ok and ratio_pm_star_neb < max_ratio_between_star_and_neb_pm_to_be_ok:
                        part_of_neb = "yes"
                    else:
                        part_of_neb = "no"
